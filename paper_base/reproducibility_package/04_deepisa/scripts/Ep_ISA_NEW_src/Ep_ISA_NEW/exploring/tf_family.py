...
from Ep_ISA_NEW.utils import get_data_resource, apply_plot_style, save_or_show, remove_if_exists
...
def annotate_tf_family(df):
    ...
    if 'tf_pair' in temp_df.columns:
        ...
        for suffix in ['1', '2']:
            temp_df = pd.merge(temp_df, ref, left_on=f"tf{suffix}", right_on="gene_symbol", how="left")
    ...
    # Case 2: Processing Single TFs (for DBD ranking)
        ...
        temp_df = pd.merge(temp_df, ref, left_on="tf", right_on="gene_symbol", how="left")
