raise ImportError("Deprecated. Ep_ISA_NEW consumes Fi-NeMo hits via load_finemo().")
import pandas as pd
...
def compile_training_data(df, 
    ...
    test_df = df[df['chrom'] == test_chrom].copy()
    train_val_pool = df[df['chrom'] != test_chrom].copy()
